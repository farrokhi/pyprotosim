...
void calc_mschapv2(char* p_identity, char* p_password, char* p_pwhash, char* p_auth, char* p_peer) {
    int identity_len;
    int password_len;
    int auth_len;
    int peer_len;
    ...
    identity_len =strlen(p_identity)/2-1;
    password_len =strlen(p_password)/2-1;
    auth_len =strlen(p_auth)/2-1;
    peer_len =strlen(p_peer)/2-1;    
    identity       =my_malloc(identity_len);
    password       =my_malloc(password_len);
    auth_challenge =my_malloc(auth_len);
    peer_challenge =my_malloc(peer_len);
    nt_response    =my_malloc(MSCHAPV2_NT_RESPONSE_LEN);
    auth_response  =my_malloc(MSCHAPV2_AUTH_RESPONSE_LEN);
    master_key     =my_malloc(MSCHAPV2_MASTER_KEY_LEN);
    fromHex(identity,p_identity);
    fromHex(password,p_password);
    fromHex(auth_challenge,p_auth);
    fromHex(peer_challenge,p_peer);    
    pwhash=atoi(p_pwhash);
    // Debugging output to confirm correct input
    dumpToHex("Identity=",identity,identity_len);
    dumpToHex("Password=",password,password_len);
    printf("PWHash=%d\n",pwhash);
    dumpToHex("Auth-Challenge=",auth_challenge,auth_len);
    dumpToHex("Peer-Challenge=",peer_challenge,peer_len);
    // Now we have our input parameters
    ret=mschapv2_derive_response(identity, identity_len,
        password, password_len, pwhash,
        auth_challenge,peer_challenge,
        nt_response, auth_response, master_key);
    dumpToHex("NT-Response=",nt_response,MSCHAPV2_NT_RESPONSE_LEN);
    dumpToHex("Auth-Response=",auth_response,MSCHAPV2_AUTH_RESPONSE_LEN);
    dumpToHex("Master-key=",master_key,MSCHAPV2_MASTER_KEY_LEN);
    if (ret!=0) {
        printf("ERROR=MS-CHAPv2\n");
    }
}
...
int main(int argc, char *argv[]) {
    ...
    if (argc > 1) {
        ...
        if (strcmp(argv[1],"mschapv2")==0) { //aes_128_cbc
            // in: identity,password,pwhash,auth_challenge,peer_challenge
            // out: nt_response,auth_response,master_key
            calc_mschapv2(argv[2],argv[3],argv[4],argv[5],argv[6]);
        }        
    }
    else {
        printf("EAP calculator V 0.3.2 Copyright (c) 2012-2013 by Sergej Srepfler\n");
        ...
        printf("mschapv2 <0xIdentity> <0xPassword> <PWHash> <0xAuth-Challenge> <0xPeer-Challenge>\n");        
    }
    return 0;
}
...
#0.3.0 - Mar 29 '13 - computeOPc separated, renamed encode/decode to encrypt/decrypt
#      - Sep 24 '13 - fixed param passing for MAC calculation (when NONCE_S is used for AKA)
